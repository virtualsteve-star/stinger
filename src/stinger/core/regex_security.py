...
class RegexSecurityValidator:
    ...
    def validate_pattern(self, pattern: str) -> Tuple[bool, str]:
        ...
        if compile_time > self.config.MAX_COMPILE_TIME_MS:
            return False, f"Compilation too slow: {compile_time}ms > {self.config.MAX_COMPILE_TIME_MS}ms"
    ...
    def safe_search(self, compiled_pattern: re.Pattern, text: str, timeout_ms: Optional[int] = None) -> Optional[re.Match]:
        """
