...
class GlobalRateLimiter:
    ...
    def __init__(self, backend: str = "memory", cleanup_interval: int = 3600, config: Optional[dict] = None, config_path: Optional[str] = None):
        """
        ...
        self.default_limits = self.config.get('default_limits', {
    ...
    def check_rate_limit(self, api_key: str, role_or_limits: Optional[Union[str, Dict[str, int]]] = None, *, role: Optional[str] = None) -> dict:
        """
        ...
        if isinstance(role_or_limits, dict):
            ...
            max_per_minute = int(max_per_minute) if max_per_minute is not None else self.max_requests_per_minute
            max_per_hour = int(max_per_hour) if max_per_hour is not None else self.max_requests_per_hour
            ...
            if max_per_minute == 0 or max_per_hour == 0:
                return {
                    'exceeded': True,
                    'key': api_key,
                    'exceeded_limits': ['requests_per_minute'] if max_per_minute == 0 else ['requests_per_hour'],
                    'remaining': {'minute': 0, 'hour': 0},
                    'limit': {'minute': max_per_minute, 'hour': max_per_hour},
                    'reason': 'Zero rate limit configured',
                    'details': {
                        'requests_per_minute': {'current': 0, 'remaining': 0, 'limit': max_per_minute},
            ...
            max_per_minute = role_config.get('max_requests_per_minute') if role_config else self.max_requests_per_minute
            max_per_hour = role_config.get('max_requests_per_hour') if role_config else self.max_requests_per_hour
