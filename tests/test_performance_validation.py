...
def test_high_volume_performance():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        def process_requests(thread_id, num_requests):
            ...
            for i in range(num_requests):
                conversation = Conversation(
                    f"user_{thread_id}_{i}", "assistant", conversation_id=f"conv_{thread_id}_{i}"
                )

                # Simulate typical guardrail pipeline usage
                input_result = pipeline.check_input(
                    f"Test message {i} from thread {thread_id}", conversation=conversation
                )

                output_result = pipeline.check_output(
                    f"Test response {i} to thread {thread_id}", conversation=conversation
                )
        ...
        ), f"Performance too low: {requests_per_second:.1f} req/s (target: 100+ req/s)"
        ...
        assert (
            len(lines) >= expected_min_events
        ), f"Expected at least {expected_min_events} events, got {len(lines)}"
...
def test_zero_blocking_validation():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        for i in range(49):
            start = time.time()
            input_result = pipeline.check_input(
                f"Test message with audit {i}", conversation=audit_conversation
            )
            output_result = pipeline.check_output(
                f"Test response with audit {i}", conversation=audit_conversation
            )
            end = time.time()
        ...
        assert (
            overhead * 1000 < 10
        ), f"Audit overhead too high: {overhead * 1000:.2f}ms (target: <10ms)"
...
def test_async_buffer_efficiency():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        for i in range(500):
            audit.log_prompt(
                f"Burst test prompt {i}", user_id=f"user_{i}", conversation_id=f"conv_{i}"
            )
