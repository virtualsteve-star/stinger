...
class TestSimpleCodeGenerationFilter:
    ...
    def basic_config(self):
        ...
        return {
            'enabled': True,
            'categories': ['code_blocks', 'programming_keywords', 'code_injection', 'file_operations', 'system_commands'],
    ...
    def filter_instance(self, basic_config):
        ...
        return SimpleCodeGenerationFilter("test_code_filter", basic_config)
    
    def test_filter_initialization(self, basic_config):
        """Test filter initialization with various configurations."""
        # Test basic initialization
        filter_instance = SimpleCodeGenerationFilter("test_filter", basic_config)
        assert filter_instance.name == "test_filter"
        assert filter_instance.guardrail_type == GuardrailType.CODE_GENERATION
        assert filter_instance.enabled is True
        assert filter_instance.confidence_threshold == 0.6
        assert filter_instance.min_keywords == 1
        assert filter_instance.on_error == 'warn'
        
        # Test with minimal config
        minimal_config = {'enabled': True}
        filter_instance = SimpleCodeGenerationFilter("minimal_filter", minimal_config)
        assert filter_instance.enabled_categories == list(filter_instance.code_patterns.keys())
        assert filter_instance.confidence_threshold == 0.6  # Default
        assert filter_instance.min_keywords == 3  # Default
        assert filter_instance.on_error == 'warn'  # Default
    ...
    def test_category_validation(self):
        ...
        filter_instance = SimpleCodeGenerationFilter("test_filter", config)
        # Should only include valid categories
        assert 'code_blocks' in filter_instance.enabled_categories
        assert 'programming_keywords' in filter_instance.enabled_categories
        assert 'unknown_category' not in filter_instance.enabled_categories
    
    @pytest.mark.asyncio
    async def test_code_blocks_detection(self, filter_instance):
    ...
    @pytest.mark.asyncio
    async def test_programming_keywords_detection(self, filter_instance):
    ...
    @pytest.mark.asyncio
    async def test_code_injection_detection(self, filter_instance):
    ...
    @pytest.mark.asyncio
    async def test_file_operations_detection(self, filter_instance):
        """Test file operation pattern detection."""
        # Valid file operation patterns
        test_cases = [
            ("file.read() and file.write()", True),  # Above threshold (0.6)
            ("mkdir directory and chmod 755", True),  # Above threshold (0.6) due to system_commands overlap
        ...
        for content, should_block in test_cases:
            ...
            if should_block:
                assert result.blocked is True  # At or above threshold
                assert result.confidence == pytest.approx(0.65) or result.confidence == pytest.approx(0.8)
    ...
    @pytest.mark.asyncio
    async def test_system_commands_detection(self, filter_instance):
    ...
    @pytest.mark.asyncio
    async def test_multiple_code_detection(self, filter_instance):
    ...
    async def test_minimum_keywords_threshold(self):
        ...
        low_filter = SimpleCodeGenerationFilter("low_keywords", low_keywords_config)
        ...
        high_filter = SimpleCodeGenerationFilter("high_keywords", high_keywords_config)
    ...
    async def test_confidence_threshold(self):
        ...
        low_filter = SimpleCodeGenerationFilter("low_threshold", low_threshold_config)
        ...
        high_filter = SimpleCodeGenerationFilter("high_threshold", high_threshold_config)
    ...
    async def test_disabled_filter(self):
        ...
        filter_instance = SimpleCodeGenerationFilter("disabled_filter", config)
        
        result = await filter_instance.analyze("```python\nprint('hello')\n```")
        assert result.blocked is False
        assert result.confidence == 0.0
        assert "disabled" in result.reason
    
    @pytest.mark.asyncio
    async def test_no_code_detection(self, filter_instance):
    ...
    def test_configuration_methods(self, filter_instance):
        ...
        assert filter_instance.min_keywords == 5
    ...
    async def test_error_handling(self):
        ...
        filter_instance = SimpleCodeGenerationFilter("error_test", config)
