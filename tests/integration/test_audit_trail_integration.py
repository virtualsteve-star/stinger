    ...
    def test_pipeline_with_conversation_audit(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            output_result1 = pipeline.check_output(
                "AI is artificial intelligence.", conversation=conversation
            )

            input_result2 = pipeline.check_input(
                "Tell me more about machine learning", conversation=conversation
            )
            output_result2 = pipeline.check_output(
                "Machine learning is a subset of AI.", conversation=conversation
            )
            ...
            assert (
                len(conversation_events) >= 4
            ), f"Should have events for both turns, found {len(conversation_events)} events"
