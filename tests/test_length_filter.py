#!/usr/bin/env python3
"""
Comprehensive tests for LengthFilter.
...
class TestLengthFilter:
    """Test suite for LengthFilter functionality."""
    ...
    def test_config_validation_failure(self):
        ...
        for config in invalid_configs:
            ...
            filter_instance = LengthFilter(valid_config)
            
            # Manually set invalid config to test validation
            filter_instance.config.update(config)
            filter_instance.action = config.get('action', 'block')
            assert filter_instance.validate_config() == False
            
        # Test that string values cause TypeError during initialization
        with pytest.raises(TypeError):
            LengthFilter({'min_length': 'not_a_number', 'on_error': 'allow'})
            
        with pytest.raises(TypeError):
            LengthFilter({'max_length': 'not_a_number', 'on_error': 'allow'})
