...
class TestBlockAction:
    ...
    def test_block_action_blocks_content(self):
        ...
        config = {
            "name": "pii_blocker",
            "config": {
                "action": "block",  # Explicitly set to block
        ...
        assert "detected" in result.reason.lower() or "pii" in result.reason.lower(), "Should explain detection"
...
class TestWarnAction:
    ...
    def test_warn_action_behavior(self):
        ...
        if result.blocked:
            assert "warn" in result.reason.lower() or result.confidence < 1.0, "Should indicate warning nature"
        else:
...
class TestAllowAction:
    ...
    def test_allow_for_analytics(self):
        ...
        for text in test_cases:
            ...
            print(f"Text: '{text[:20]}...' - blocked={result.blocked}, confidence={result.confidence:.2f}")
