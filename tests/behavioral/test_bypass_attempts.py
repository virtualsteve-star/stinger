...
class TestPIIBypassAttempts:
    ...
    def test_pii_format_variations(self):
        ...
        config = {
            "name": "pii_robust",
            "config": {"confidence_threshold": 0.6, "patterns": ["ssn", "credit_card"]},
        }
    ...
    def test_credit_card_obfuscation(self):
        ...
        config = {
            "name": "cc_robust",
            "config": {"confidence_threshold": 0.6, "patterns": ["credit_card"]},
        }
        guardrail = SimplePIIDetectionGuardrail("test", config)
...
class TestURLBypass:
    ...
    def test_homograph_domains(self):
        ...
        config = {
            "name": "homograph_test",
            "config": {"blocked_domains": ["google.com", "microsoft.com"], "action": "block"},
        }
        guardrail = URLGuardrail(config)
