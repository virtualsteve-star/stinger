...
class TestSimplePIIDetectionFilter:
    ...
    def filter_instance(self, basic_config):
        ...
        return SimplePIIDetectionFilter("test_pii_filter", basic_config)
    
    def test_filter_initialization(self, basic_config):
        """Test filter initialization with various configurations."""
        # Test basic initialization
        filter_instance = SimplePIIDetectionFilter("test_filter", basic_config)
        assert filter_instance.name == "test_filter"
        assert filter_instance.guardrail_type == GuardrailType.PII_DETECTION
        ...
        filter_instance = SimplePIIDetectionFilter("minimal_filter", minimal_config)
        assert filter_instance.enabled_patterns == list(filter_instance.pii_patterns.keys())
        assert filter_instance.confidence_threshold == 0.8  # Default
    ...
    def test_pattern_validation(self):
        ...
        config = {
            'enabled': True,
            'patterns': ['ssn', 'unknown_pattern', 'email']
        }
        filter_instance = SimplePIIDetectionFilter("test_filter", config)
        # Should only include valid patterns
        assert 'ssn' in filter_instance.enabled_patterns
        assert 'email' in filter_instance.enabled_patterns
        assert 'unknown_pattern' not in filter_instance.enabled_patterns
    
    @pytest.mark.asyncio
    async def test_ssn_detection(self, filter_instance):
    ...
    @pytest.mark.asyncio
    async def test_credit_card_detection(self, filter_instance):
    ...
    @pytest.mark.asyncio
    async def test_email_detection(self, filter_instance):
    ...
    @pytest.mark.asyncio
    async def test_phone_detection(self, filter_instance):
    ...
    @pytest.mark.asyncio
    async def test_ip_address_detection(self, filter_instance):
    ...
    @pytest.mark.asyncio
    async def test_multiple_pii_detection(self, filter_instance):
    ...
    async def test_confidence_threshold(self):
        ...
        low_filter = SimplePIIDetectionFilter("low_threshold", low_threshold_config)
        ...
        high_filter = SimplePIIDetectionFilter("high_threshold", high_threshold_config)
    ...
    async def test_disabled_filter(self):
        ...
        filter_instance = SimplePIIDetectionFilter("disabled_filter", config)
        
        result = await filter_instance.analyze("SSN: 123-45-6789")
        assert result.blocked is False
        assert result.confidence == 0.0
        assert "disabled" in result.reason
    
    @pytest.mark.asyncio
    async def test_no_pii_detection(self, filter_instance):
    ...
    def test_configuration_methods(self, filter_instance):
        ...
        assert filter_instance.confidence_threshold == 0.9
    ...
    async def test_error_handling(self):
        ...
        filter_instance = SimplePIIDetectionFilter("error_test", config)
