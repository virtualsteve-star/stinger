...
from stinger.guardrails.regex import RegexGuardrail
from stinger.guardrails.length import LengthGuardrail
...
def keyword_guardrail() -> KeywordListGuardrail:
    ...
    return KeywordListGuardrail({
        'keywords': ['test', 'hack', 'exploit'],
        'match_type': 'exact'
    })
        ...
        if expected_reasons is not None:
            if isinstance(expected_reasons, list):
                for reason in expected_reasons:
                    assert any(reason in r for r in result.reasons), f"Expected reason '{reason}' not found in {result.reasons}"
            else:
        ...
        if expected_warnings is not None:
            if isinstance(expected_warnings, list):
                for warning in expected_warnings:
                    assert any(warning in w for w in result.warnings), f"Expected warning '{warning}' not found in {result.warnings}"
            else:
