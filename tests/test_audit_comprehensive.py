...
def test_comprehensive_security_audit_trail():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        conversation = Conversation("forensic_user", "ai_assistant", conversation_id="forensic_conv")
        ...
        output_result1 = pipeline.check_output("Machine learning is a subset of AI that enables computers to learn.", conversation=conversation)
        assert output_result1 is not None
        
        # Input 2: Prompt with PII (should be redacted)
        input_result2 = pipeline.check_input("My email is john@company.com, can you help?", conversation=conversation)
        assert input_result2 is not None
        
        # Output 2: Response
        output_result2 = pipeline.check_output("I can help, but I won't store your email.", conversation=conversation)
        ...
        assert len(lines) >= 8  # enable + prompt + decisions + response + prompt + decisions + response
        ...
        conversation_specific = [e for e in conv_events if e.get("conversation_id") == "forensic_conv"]
