...
async def test():
    filter = PromptInjectionGuardrail(
        "test", {"risk_threshold": 70, "block_levels": ["high", "critical"], "on_error": "allow"}
    )
    ...
    for content in test_content:
        ...
        print(
            f"Blocked: {result.blocked}, Risk: {result.risk_level}, Confidence: {result.confidence:.2f}, Reason: {result.reason}"
        )
